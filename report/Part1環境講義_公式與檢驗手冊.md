...
> 最後更新：2026-07-16（全 §0–§12 逐節查證；三個環境參數修正 ✅）
...
### ① 這是什麼
NLOS 的反射從哪來——散射點**貼在大樓面向街道的牆面上**（每面牆取樣 5 點、向外推 `config.scatterOffsetDist` m、落在路廊內才保留），**決定性生成（無 rand）**。決定性是相位連續 $\varphi = 2\pi(d_1+d_2)/\lambda$ 的前提：散射點若每步亂跳，相位就斷了。

#### 🔴 **散射點外推距離說明（2026-07-13 補正）**

當前設定 `scatterOffsetDist = 0.5 m`（見 `config.m`），**無 IEEE/3GPP 文獻規範**，屬工程設計選擇：

- **設計理由**：幾何散射建模慣例上，散射點不貼牆表面而在牆外邊界
  - 物理含義：反射信號發源於牆外空間（避免邊界效應、模擬 Fresnel 反射層位置）
  - 推測基礎（未驗證）：3GPP TR 38.901 穿透損失 20–30 dB → 牆厚 ~0.2–0.3 m → 反射層可能位於此範圍內外
  
- **文獻缺陷**
  - 3GPP TR 38.901：未規範散射點與牆面的偏移距離
  - IEEE 期刊：幾何散射建模多採「固定偏移」但無統一標準（2026-07-13 網查驗證 ✅）
  - Shakya2025：FR3 實測，未提及此參數

- **⚠️ 分類**：**[T3 工程設計，無文獻直接支持]**（2026-07-13 網查驗證確認）
  
- **敏感度未測**：0.5 m 選擇對 SNR、波束對齊度、側街連線率的影響**未量化**

#### **待驗證計畫（Q3 2026-09，見 SPEC.md §1.2a）**

與 Sloss、K 值同並行。執行清單：

1. **敏感度掃描**：跑消融實驗 `scatterOffsetDist ∈ {0, 0.3, 0.5, 1.0}` m
   - 測評指標：SNR 分布、波束對齊度、側街連線成功率、收斂速度
   - 預期結果：若差異 <0.5 dB 則保留 0.5 m；否則調整或參數化為 f(fc)
   
2. **頻率無關性驗證**：檢查 0.5 m 是否對 7–24 GHz 全 FR3 頻段適用
   - Fraunhofer 遠場邊界：2D²/λ ≈ 2.8 m @ 16.95 GHz → 0.5 m ≪ 2.8 m（遠場成立）
   - 但若換至 7 GHz，遠場 ~14 m；若換至 24 GHz，遠場 ~1.6 m → 可考慮 offsetDist = f(fc)

### ② 公式
牆面取樣點 $\mathbf{s} = \mathbf{w}(u) + d_{offset}\,\hat{\mathbf{n}}$，其中 $d_{offset} = $`scatterOffsetDist`（$u \in \{0.1,\dots,0.9\}$ 牆面參數、$\hat{\mathbf{n}}$ 外法向），保留條件 $\mathbf{s}\in\mathcal{R}$（道路走廊）。通道只取距車最近的 **top-K**（現設 K=3）：

$$
\mathcal{S}_{act} = \underset{|\mathcal{S}'|=K}{\arg\min} \sum_{\mathbf{s}\in\mathcal{S}'} \lVert \mathbf{s}-\mathbf{x}_{veh}\rVert
$$

> ⚠️ **K=3 待驗證補充**（2026-07-12）
> 
> **現狀**：K=3 為暫定設計，未經量化驗證
> 
> **推導基礎**：
> - [T1] Shakya2025 觀察「FR3 稀疏，少數主導 MPC」（ASA 15.3° ≪ 3GPP 47.3°）
> - [T3] 推導理由：高頻波束窄（HPBW ~3.5° @ 128-UPA）→ 接收少數路徑
> - [T3] 工程選擇：K=3 兼顧精度與計算成本
> 
> **文獻缺陷**：Shakya2025 論文未明確給出「K=?」的量化值
> 
> **敏感度未測**：K=1（無 NLOS）vs K=3（當前）vs K=5（中等稀疏）性能差異未知
> 
> **待驗證計畫**（Q3 2026-09）：
> 1. 複查 Shakya2025 MPC 計數統計（每位置平均幾條 MPC？）
> 2. 消融實驗：跑 200 eval episodes，K∈{1, 3, 5} 對比收斂曲線 & 最終 reward
> 3. 配置參數化：config.m 新增 `numMpcNlos=3` 含註解說明暫定狀態

### ②½ 程式對應
| 內容 | 檔案 | 位置 |
|---|---|---|
| 牆面取樣+外推 `scatterOffsetDist`+落路保留 | `env/generateScatterers.m` | 主迴圈（每牆 5 點、linspace 0.1–0.9 避角；第 35–50 行） |
| `scatterOffsetDist` 參數定義 | `config.m` | 新增 §Scattering Model Parameters (2026-07-13)；值 0.5 m，標註 [T3 工程設計，Q3 待驗證] |
...
- 牆面散射建模：**幾何式決定性單次反射模型（geometry-based deterministic / single-bounce ray-tracing）+ point-scatterer 近似**（2026-07-16 更正，前身誤標「GBSM」——GBSM 的 S=Stochastic 指隨機撒群集，與本專題「決定性、無 rand、貼真實牆面」相反）。出處：**GEMV²（Boban et al., IEEE TVT 2014，§2 已引）**以真實建物輪廓做決定性 LOS/NLOS，與本法最貼合；ETSI TR 103 257-1（V2X 決定性 ray-tracing）；point-scatterer 近似見 arXiv:2511.20572。
...
> 🟡 **數值出處註記**：$G_{mf}=10$ 的**公式形式**出自 Zhao2024 Eq.6（匹配濾波增益），但**數值「10」在庫內查無**（Zhao2024 Table 1 為圖片、未轉文字）——目前僅專案報告自述，**待對回 Zhao2024 原表核實**。
